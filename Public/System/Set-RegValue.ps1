function Set-RegValue
{
    ...
    To store integer values greater than `[Int32]::MaxValue` or `[Int64]::MaxValue`, use the `UDWord` and `UQWord` parameters, respectively, which are unsigned integers.
    ...
    .EXAMPLE
    Set-RegistryKeyValue -Path 'hklm:\Software\xSecOps\Test -Name Status -String foobar
     
    Creates the `Status` string value under the `hklm:\Software\xSecOps\Test` key and sets its value to `foobar`.
     
    .EXAMPLE
    Set-RegistryKeyValue -Path 'hklm:\Software\xSecOps\Test -Name ComputerName -String '%ComputerName%' -Expand
     
    Creates an expandable string. When retrieving this value, environment variables will be expanded.
     
    .EXAMPLE
    Set-RegistryKeyValue -Path 'hklm:\Software\xSecOps\Test -Name Movies -String ('Signs','Star Wars','Raiders of the Lost Ark')
     
    Sets a multi-string (i.e. array) value.
     
    .EXAMPLE
    Set-RegistryKeyValue -Path hklm:\Software\xSecOps\Test -Name 'SomeBytes' -Binary ([byte[]]@( 1, 2, 3, 4))
     
    Sets a binary value (i.e. `REG_BINARY`).
     
    .EXAMPLE
    Set-RegistryKeyValue -Path hklm:\Software\xSecOps\Test -Name 'AnInt' -DWord 48043
     
    Sets a binary value (i.e. `REG_DWORD`).
     
    .EXAMPLE
    Set-RegistryKeyValue -Path hklm:\Software\xSecOps\Test -Name 'AnInt64' -QWord 9223372036854775807
     
    Sets a binary value (i.e. `REG_QWORD`).
     
    .EXAMPLE
    Set-RegistryKeyValue -Path hklm:\Software\xSecOps\Test -Name 'AnUnsignedInt' -UDWord [uint32]::MaxValue
     
    Demonstrates how to set a registry value with an unsigned integer or an integer bigger than `[int]::MaxValue`.
 
    The `UDWord` parameter was added in xSecOps 2.0. In earlier versions of xSecOps, you have to convert the unsigned int's bits to a signed integer:
    ...
    .EXAMPLE
    Set-RegistryKeyValue -Path hklm:\Software\xSecOps\Test -Name 'AnUnsignedInt64' -UQWord [uint64]::MaxValue
     
    Demonstrates how to set a registry value with an unsigned 64-bit integer or a 64-bit integer bigger than `[long]::MaxValue`.
 
    The `UQWord parameter was added in xSecOps 2.0. In earlier versions of xSecOps, you have to convert the unsigned int's bits to a signed integer:
    ...
    .EXAMPLE
    Set-RegistryKeyValue -Path hklm:\Software\xSecOps\Test -Name 'UsedToBeAStringNowShouldBeDWord' -DWord 1 -Force
...
Export-ModuleMember -Function Set-RegValue